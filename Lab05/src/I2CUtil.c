...
void I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
	...
	ESP_ERROR_CHECK(i2c_master_start(cmd));                // Start command
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd, (address << 1) | I2C_MASTER_WRITE, 1));
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd, reg, 1));   // Write to register
	ESP_ERROR_CHECK(i2c_master_write_byte(cmd, data, 1));  // Write data
	ESP_ERROR_CHECK(i2c_master_stop(cmd));                 // End command
	ESP_ERROR_CHECK(i2c_master_cmd_begin(I2C_NUM_0, cmd, TIMEOUT_DELAY / portTICK_RATE_MS));
